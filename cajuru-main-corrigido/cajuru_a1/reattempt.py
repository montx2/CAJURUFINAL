...
def reattempt_locked(result: PipelineResult, cfg: dict, clientes: list[JetaxClient]) -> PipelineResult:
    ...
    pfx_timeout = int(security.get("timeout_pfx_segundos", 90))
    ...
    # Usa o mesmo isolamento de processo do pipeline: se o OpenSSL travar
    # num PFX corrompido durante a segunda passagem, esse arquivo vai para
    # revisão manual sem derrubar o job inteiro.
    with IsolatedPfxInspector(timeout=pfx_timeout) as runner:
        for cert in locked:
            names: list[str] = []
            ...
            new = runner.inspect(
                ...
                max_bytes=max_bytes, max_attempts=max_attempts,
