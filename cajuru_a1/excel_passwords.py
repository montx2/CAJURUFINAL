"""Leitura resiliente das planilhas de senhas."""
...
from cajuru_a1.cnpjutil import extract_docs_from_text, is_valid_doc, only_digits, pad_cnpj
...
NAME_HEADERS = {_header_key(x) for x in (
    "EMPRESAS", "EMPRESA", "RAZAO", "RAZÃO SOCIAL", "NOME", "CLIENTE", "CLIENTES",
    "RAZAOSOCIAL", "NOME EMPRESARIAL",
)}
PASS_HEADERS = {_header_key(x) for x in (
    "SENHA", "SENHAS", "PASSWORD", "PASSWD", "SENHA CERTIFICADO", "SENHA A1",
    "SENHA DO CERTIFICADO",
)}
CNPJ_HEADERS = {_header_key(x) for x in (
    "CNPJ", "CNPJ/CPF", "CPF", "DOCUMENTO", "CNPJ CPF", "CPF/CNPJ",
)}
...
def _read_sheet(path: Path, sheet, vault: PasswordVault, say) -> int:
    ...
    for row_number, row in enumerate(sheet.iter_rows(values_only=True), 1):
        ...
        if columns is None:
            if row_number <= 100:
                ...
                continue
            vault.finding("warning", "cabecalho_ausente",
                          "Aba ignorada: cabeçalho de empresa e senha não encontrado nas primeiras 100 linhas",
                          origem=str(path), aba=sheet.title)
            return 0
        if row_number <= header_row:
            continue
        name = _cell_text(_value(row, columns.get("nome")))
        ...
        if doc_raw:
            ...
            if is_valid_doc(candidate):
                ...
                if len(docs) == 1:
                    ...
                    vault.finding("warning", "documento_invalido_excel",
                                  "Documento inválido ou ambíguo ignorado",
                                  origem=str(path), aba=sheet.title, linha=row_number)
    ...
    if columns is None:
        vault.finding("warning",
                      "aba_vazia" if rows_seen == 0 else "cabecalho_ausente",
                      "Aba vazia ignorada" if rows_seen == 0 else "Aba ignorada: cabeçalho não encontrado",
                      origem=str(path), aba=sheet.title)
        return 0
    if empty_passwords:
        vault.finding("warning", "senhas_vazias", "Linhas com senha vazia ignoradas",
                      origem=str(path), aba=sheet.title, quantidade=empty_passwords)
    if numeric_passwords:
        vault.finding("warning", "senhas_numericas",
                      "Senhas armazenadas como número podem ter perdido zeros à esquerda; formate como texto",
                      origem=str(path), aba=sheet.title, quantidade=numeric_passwords)
