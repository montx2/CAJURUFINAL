"""Validação, normalização e extração de CNPJ/CPF."""
...
def pad_cnpj(digits: str) -> str:
    """Completa zeros à esquerda em CNPJs que perderam o zero inicial."""
...
def extract_docs_from_text(text: str, *, allow_missing_leading_zero: bool = True) -> list[str]:
    """Extrai CNPJs/CPFs válidos de um texto sem concatenar grupos independentes."""
    if not text:
        return []
    found: list[str] = []
    seen: set[str] = set()
...
def best_doc_from_filename(filename: str) -> str | None:
    """Retorna o documento do nome do arquivo, se houver exatamente um."""
