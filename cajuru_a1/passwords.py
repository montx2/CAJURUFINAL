"""Cofre de senhas em memória e geração restrita de candidatas."""
...
def year_suffixes(extra: list[str] | None = None) -> list[str]:
...
def pattern_passwords(empresa: str, years: list[str] | None = None) -> list[tuple[str, str]]:
...
class PasswordVault:
    ...
    def add(self, entry: PasswordEntry) -> None:
        ...
        if doc and not is_valid_doc(doc):
            self.finding("warning", "documento_invalido", "Documento inválido ignorado",
                         origem=entry.origem, aba=entry.aba, linha=entry.linha)
            doc = ""
        ...
        normalized = PasswordEntry(entry.empresa, pwd, doc, entry.validade, entry.origem, entry.aba, entry.linha)
        key = name_key(company)
        existing = self.by_key.get(key, [])
        if existing and any(item.senha != pwd for item in existing):
            self.finding("warning", "senhas_conflitantes_nome",
                         "A mesma empresa possui senhas diferentes; ambas serão testadas",
                         empresa=entry.empresa)
        if doc and self.by_cnpj.get(doc) and any(item.senha != pwd for item in self.by_cnpj[doc]):
            self.finding("high", "senhas_conflitantes_documento",
                         "O mesmo documento possui senhas diferentes", cnpj=doc)
        self.entries.append(normalized)
        self.by_key.setdefault(key, []).append(normalized)
        if doc:
            self.by_cnpj.setdefault(doc, []).append(normalized)
...
def candidate_passwords(
    *,
    ...
    for name in [empresa, *(extra_names or [])]:
        ...
        if norm and norm not in {normalize_name(x) for x in names}:
            names.append(name)
    for name in names:
    ...
    for name in names:
        for password, source in pattern_passwords(name, years):
            add(password, source)
    if include_common:
        for password in ("123456", "12345678", "123456789", "1234", "12345"):
