"""Inspeção de certificados PKCS#12 (.pfx/.p12): abertura, identidade e validade."""
...
class PfxInfo:
    ...
    def cnpj(self) -> str | None:
        # Identidade interna do X.509 sempre prevalece sobre o nome do arquivo.
...
def extract_identity(cert: x509.Certificate) -> tuple[str | None, str | None, list[str]]:
    """Retorna (empresa, documento único, evidências) sem dados secretos."""
    evidence: list[str] = []
    companies: list[str] = []
    cnpjs: list[str] = []
    cpfs: list[str] = []
    ...
    for oid, label in ((NameOID.ORGANIZATION_NAME, "organizationName"),
    ...
    # e-CNPJ contém o CPF do responsável e o CNPJ da empresa: CNPJ prevalece.
    if len(cnpjs) == 1:
        document = cnpjs[0]
...
def try_open_pfx(data: bytes, password: str | None):
    try:
...
def inspect_file(source: Path, candidates: list[tuple[str, str]], *,
                 max_bytes: int = 30 * 1024 * 1024, max_attempts: int = 500) -> PfxInfo:
    source = Path(source)
    try:
    ...
    password, password_source, key, certificate, attempts, error_code = unlock_pfx(
        source, candidates, max_bytes=max_bytes, max_attempts=max_attempts,
    )
    info.attempts = attempts
    if certificate is None:
        info.error_code = error_code
        info.error = _ERROR_MESSAGES.get(error_code, "Não foi possível abrir o certificado")
        return info
    ...
    # O CNPJ interno do X.509 é a identidade oficial. Divergência com o nome do
    # arquivo é comum (nomes sujos) e NÃO bloqueia — apenas registramos como
    # evidência para o relatório.
    if info.cnpj_filename and info.cnpj_cert and info.cnpj_filename != info.cnpj_cert:
        info.extra["aviso_nome_divergente"] = (
            f"CNPJ do nome ({info.cnpj_filename}) difere do interno ({info.cnpj_cert}); "
            "usado o CNPJ interno do certificado."
        )
...
def pick_newest(certs: list[PfxInfo]) -> tuple[PfxInfo | None, list[PfxInfo]]:
    """Para vários PFX do mesmo CNPJ, devolve (escolhido, substituídos).

    Escolhe o certificado mais novo e válido; os demais ficam como substitutos.
    """
